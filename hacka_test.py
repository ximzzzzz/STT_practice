...
parser.add_argument('--test-path', metavar='DIR',
                    help='path to test ', default='./test/')
parser.add_argument('--gpu', dest='cuda', action='store_true', help='Use cuda to train model')
...
parser.add_argument('--continue-from', default='./ckpt_22_12000.pth', help='Continue from checkpoint model')
...
def test(args, path_list, device):
    ...
    test_dataset_ = SpecDatasetTest(audio_conf, path_list, labels)
    test_dataloader_ = AudioDataLoaderTest(test_dataset_, num_workers = args.num_workers, batch_size = args.batch_size)
    print('Validation Start')
    with torch.no_grad():
        for data in tqdm(test_dataloader_, total=len(test_dataloader_)):
            inputs, input_percentages, paths = data
            ...
            del(z); del(updated_lengths); 
            torch.cuda.empty_cache()
            
            decoded_output, _ = target_decoder.decode(asr_out, asr_out_sizes)
            decoded_output = [x[0].replace(' ', '') for x in decoded_output]
...
if __name__=='__main__':
    ...
    path_list = get_pcm_files(args.test_path)
    device = torch.device("cuda" if args.cuda else "cpu")
    test(args, path_list[:50], device)
