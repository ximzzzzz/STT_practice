...
#             print("using accents")
            accentData = self.accent[sample[2]]
#             return spect,transcript,accentData
            return spect, transcript, audio_path , accentData
...
#         transcript = [self.label_map.get(x) for x in list(transcript_str)]
        return transcript
...
class SpectrogramDataset_clf(SpectrogramDataset):
    def __init__(self, audio_conf, manifest_filepath, labels, normalize, speed_volume_perturb, spec_augment):
        super(SpectrogramDataset_clf, self).__init__(audio_conf, manifest_filepath, labels, normalize, speed_volume_perturb, spec_augment)
...
#         print(f'transcript : {transcript_str}')
        ...
        return spect, transcript, index
    
def _collate_fn_clf(batch):
    def func(p):
        return p[0].size(1)
    batch = sorted(batch, key=lambda sample : sample[0].size(1), reverse=True)
    longest_sample = max(batch, key=func)[0]
    ...
    input_percentages = torch.FloatTensor(minibatch_size)
    target_sizes = torch.IntTensor(minibatch_size)
    index_list = torch.IntTensor(minibatch_size)
    targets = []
    for x in range(minibatch_size):
        sample = batch[x]
        tensor = sample[0]
...
#     print(f'targets : {targets}')
    targets = torch.IntTensor(targets)
    return inputs, targets, input_percentages, index_list

class AudioDataLoader_clf(DataLoader):
    def __init__(self, *args, **kwargs):
        super(AudioDataLoader_clf, self).__init__(*args, **kwargs)
        self.collate_fn = _collate_fn_clf
        
        
class SpectrogramDatasetMix(Dataset, SpectrogramParser):
    def __init__(self, audio_conf, data_list, labels, accent=None, normalize=False, 
                 speed_volume_perturb=False, spec_augment=False):

        self.ids = data_list
        self.size = len(data_list)
        self.label_map = dict([(labels[i], i ) for i in range(len(labels))])
        super(SpectrogramDatasetMix, self).__init__(audio_conf, normalize, speed_volume_perturb, spec_augment)
        self.accent = accent
